...
// Decode a 4bpp tile from RomAssets into an 8x8 array of palette indices.
...
void FontRenderer::blit_glyph_sdl(SDL_Renderer* renderer, int tile_id, int x, int y, uint32_t color) const {
    ...
    for (int row = 0; row < 8; ++row) {
        for (int col = 0; col < 8; ++col) {
            ...
            // Use the passed color for glyph pixels (indices 3/4), preserve palette for others
            if (id == 3 || id == 4) {
...
void FontRenderer::draw_text(SDL_Renderer* renderer, int x, int y, const char* text, uint32_t color) const {
...
void FontRenderer::draw_text_right(SDL_Renderer* renderer, int x, int y, const char* text, uint32_t color) const {
    int w = text_width(text);
    draw_text(renderer, x - w, y, text, color);
}

int FontRenderer::text_width(const char* text) const {
    if (!text) return 0;
    int len = 0;
    while (text[len]) ++len;
